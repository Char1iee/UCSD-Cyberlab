...
y_pred_xgb_proba = xgb.predict_proba(x_test) # predict_proba finds probabilities of the classes, which is required for the ROC AUC score

print("XG Boost: \n Accuracy: " + str(accuracy_score(y_test, y_pred_xgb)) + " F1 score: " + str(f1_score(y_test, y_pred_xgb,average='weighted')) + " Precision: ", str(precision_score(y_test, y_pred_xgb, average='weighted')))
print("AUC Score: " + str(roc_auc_score(y_test, y_pred_xgb_proba, multi_class='ovo', average='weighted')))
matrixxgb = confusion_matrix(y_test,y_pred_xgb)
print(matrixxgb)
...
avg_TPR, avg_FPR, avg_FNR, avg_TNR = calculate_classification_metrics(matrixxgb)
