...
y_pred_gnb_proba = gnb.predict_proba(x_test) # predict_proba finds probabilities of the classes, which is required for the ROC AUC score

print("Naive Bayes: \n Accuracy: " + str(accuracy_score(y_test, y_pred_gnb)) + " F1 score: " + str(f1_score(y_test, y_pred_gnb,average='weighted')) + " Precision: ", str(precision_score(y_test, y_pred_gnb, average='weighted')))
print("AUC Score: " + str(roc_auc_score(y_test, y_pred_gnb_proba, multi_class='ovo', average='weighted')))
...
avg_TPR, avg_FPR, avg_FNR, avg_TNR = calculate_classification_metrics(matrixgnb)
