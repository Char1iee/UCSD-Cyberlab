...
y_pred_rf_proba = rf.predict_proba(x_test) # predict_proba finds probabilities of the classes, which is required for the ROC AUC score

print("Random Forest: \n Accuracy: " + str(accuracy_score(y_test, y_pred_rf)) + " F1 score: " + str(f1_score(y_test, y_pred_rf,average='weighted')) + " Precision: ", str(precision_score(y_test, y_pred_rf, average='weighted')))
print("AUC Score: " + str(roc_auc_score(y_test, y_pred_rf_proba, multi_class='ovo', average='weighted')))
matrixrf = confusion_matrix(y_test,y_pred_rf)
print(matrixrf)
...
avg_TPR, avg_FPR, avg_FNR, avg_TNR = calculate_classification_metrics(matrixrf)
